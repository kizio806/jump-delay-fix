...
public final class FabricHudEditorScreen extends Screen {
    ...
    private void drawHudPreview(DrawContext context, JumpDiagnostics diagnostics, JumpRuntimeConfig config) {
        ...
        try (FabricMatrixStackCompat.ScopedMatrixTransform transform =
                     FabricMatrixStackCompat.pushTranslateScale(context, previewHudX, previewHudY, config.hudScale())) {
            if (!transform.isActive()) {
                return;
            }
            context.fill(-3, -3, HudPresentationService.PANEL_WIDTH, model.panelHeight(), 0xA0000000);
            context.fill(-3, -3, HudPresentationService.PANEL_WIDTH, -2, 0xFF56D8FF);
            context.drawText(this.textRenderer, Text.literal(HudPresentationService.HUD_TITLE), HudPresentationService.TEXT_X, 5, 0x80FF9D, true);
            ...
            if (model.showQualityBar()) {
                ...
                if (filledWidth > 0) {
                    context.fill(barX, barY, barX + filledWidth, barY + barHeight, 0xFF000000 | model.qualityColor());
                }
            }
        }
    }
